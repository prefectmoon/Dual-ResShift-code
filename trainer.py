...
class TrainerBase:
    ...
    def build_dataloader(self):
        ...
        datasets = {'train': load_data(
            hr_data_dir=self.configs.data.train.gt_data_dir,
            lr_data_dir=self.configs.data.train.lq_data_dir,
            other_data_dir=self.configs.data.train.gbvs_data_dir), }
        self.data_dict = datasets['train'].data_dict
        ...
        if hasattr(self.configs.data, 'val') and self.rank == 0:
            ...
            datasets['val'] = load_data(
            hr_data_dir=self.configs.data.val.gt_data_dir,
            lr_data_dir=self.configs.data.val.lq_data_dir,
            other_data_dir=self.configs.data.val.gbvs_data_dir)
        if self.rank == 0:
    ...
    def _extract_batch(self, mean, sd):
        X = self.get_truncated_normal(mean=mean, sd=sd, low=self.lowest, upp=self.highest)
        stats = Counter(np.round(X.rvs(self.configs.train.batch[0])))

        batch_idx_list = []
        for j in stats.keys():
            sample_size = min(stats[j], len(self.data_dict[j]))
            batch_idx = np.random.choice(self.data_dict[j], size=sample_size, replace=False)
            for index in batch_idx:
                batch_idx_list.append(index)

        hr_data, lr_data, other_data = self.datasets['train'][batch_idx_list]

        hr_data, lr_data, other_data = self.get_normalize(hr_data),self.get_normalize(lr_data),self.get_normalize(other_data)
        hr_data = (hr_data - 0.5) / 0.5  # [0, 1] to [-1, 1]
        lr_data = (lr_data - 0.5) / 0.5  # [0, 1] to [-1, 1]
        other_data = (other_data - 0.5) / 0.5  # [0, 1] to [-1, 1]
        # hr_data1, lr_data1, other_data1 = self.datasets['train'][batch_idx_list]
        #hr_data1, lr_data1, other_data1 = self.get_normalize(hr_data1),self.get_normalize(lr_data1),self.get_normalize(other_data1)
        # hr_data1 = (hr_data1 - 0.5) / 0.5  # [0, 1] to [-1, 1]
        # lr_data1 = (lr_data1 - 0.5) / 0.5  # [0, 1] to [-1, 1]
        # other_data1 = (other_data1 - 0.5) / 0.5  # [0, 1] to [-1, 1]
        hr_data, lr_data, other_data, flag_nan = replace_nan(hr_data, lr_data, other_data)
        # vutils.save_image(lr_data[6], 'E:/ias/ias_process/lr_'+str(6) + '.jpg', normalize=True)
        if flag_nan:
        ...
        #         'other': other_data}
        data = {'gt': hr_data, 'lq': lr_data, 'other': other_data}
        return data
    ...
    def train(self):
        ...
        mean = self.lowest
        sd = 0.5
        self.current_iters = self.configs.train.current_step
        for ii in range(self.iters_start, self.configs.train.iterations):
            self.current_iters = ii + 1

            # prepare data return {'lq':self.lq, 'gt':self.gt} return {key:value.cuda().to(dtype=dtype) for key, value in data.items()}
            # data = self.prepare_data(next(self.dataloaders['train']))
            # print("prepare data", data)

            data = self._extract_batch(mean=mean, sd=sd)
            mean = self.lowest * (1 - ii / self.configs.train.iterations) + self.highest * ii / self.configs.train.iterations
...
class TrainerDifIR(TrainerBase):
    ...
    def diag_sym_matrix(slef, k=256):
    ...
    def gbvs_process(self, image_tensor):
        processed_images = []

        for index in range(len(image_tensor)):
            image = image_tensor[index]
            tensor_cpu = image.cpu()
            image = tensor_cpu.permute(1, 2, 0).numpy() #64,64,3
            image = (image * 255).astype(np.uint8) #image = (image * 255).to(torch.uint8)
            pil_image = np.array(image, dtype=np.float32)
            pil_image[np.isnan(pil_image)] = 1
            image_fixed = pil_image.astype(np.uint8)
            '''
            image_tensor = torch.tensor(image, dtype=torch.float32)
            image_tensor[image_tensor.isnan()] = 1
            image_fixed = image_tensor.to(torch.uint8)
            '''

            _saliency_map_gbvs = gbvs.compute_saliency(image_fixed)
            # _saliency_map_gbvs = np.repeat(saliency_map_gbvs[:, :, np.newaxis], 3, axis=2)

            _candy = cv2.Canny(image, 0, 50)
            r, g, b = cv2.split(image)
            add = (r + g + b) / 3
            saliency_map_gbvs2 = _saliency_map_gbvs + add
            saliency_map_gbvs2 = np.repeat(saliency_map_gbvs2[:, :, np.newaxis], 3, axis=2)

            saliency_map_gbvs2[:, :, 0] += _candy  # Red通道
            saliency_map_gbvs2[:, :, 1] += _candy  # Green通道
            saliency_map_gbvs2[:, :, 2] += _candy  # Blue通道
            edge_image_tensor = TF.to_tensor(saliency_map_gbvs2)

            processed_images.append(edge_image_tensor)
        # 重新组成张量
    ...
    def prepare_data(self, data, dtype=torch.float32, realesrgan=None, phase='train'):
        ...
        if realesrgan and phase == 'train':
            ...
            # self.other = self.edge_process(self.lq).cuda()
            # self.other = self.gbvs_process(self.lq).cuda()


            return {'lq': self.lq, 'gt': self.gt, 'other': self.other}
        else:
            # self.other = self.gbvs_process(self.lq).cuda()
            # data['other'] = self.other.cuda()
    ...
    def training_step(self, data):
        ...
        for jj in range(0, current_batchsize, micro_batchsize):
            ...
            latent_downsamping_sf = 1**(len(self.configs.autoencoder.params.ddconfig.ch_mult) - 1)
            ...
            model_kwargs={'lq':micro_data['lq'],'other':micro_data['other']}
            ...
            print("losses, tt, micro_data, z_t, z0_pred, last_batch:", losses, jj)
    ...
    def log_step_train(self, loss, tt, batch, z_t, z0_pred, flag=False, phase='train'):
        ...
        if self.rank == 0:
            ...
            if self.current_iters % self.configs.train.log_freq[0] == 0 and flag:
                ...
                log_str = '{} : Train: {:06d}/{:06d}, Loss/MSE/ms_ssim_val/L1: '.format(
                        current_datetime,
                        self.current_iters,
                        self.configs.train.iterations)
                for jj, current_record in enumerate(record_steps):
                    log_str += 't({:d}):{:.2e}/{:.2e}/{:.2e}/{:.2e}, '.format(
                            current_record,
                            self.loss_mean['loss'][jj].item(),
                            self.loss_mean['mse'][jj].item(),
                            self.loss_mean['ms_ssim_val'][jj].item(),
                            self.loss_mean['l1_loss'][jj].item(),
                            )
                    # tensorboard
    ...
    def encode_first_stage(self, y, first_stage_model, up_sample=False):
        ...
        if first_stage_model is not None:
            with th.no_grad():
                ...
                y = z_y * self.configs.train.get('sf')
                y = y.type(ori_dtype)

        if up_sample:
            y = F.interpolate(y, scale_factor=self.configs.train.get('sf'), mode='bicubic')
    ...
    def validation(self, phase='val'):
        if self.rank == 0:
            ...
            for ii, data in enumerate(self.dataloaders[phase]):#image,lq
                # data = {key:value.cuda().to(dtype=torch.float32) for key, value in data.items()}

                data = {"gt": data[0], "lq": data[1], 'other': data[2]}
                if 'gt' in data:
                    im_lq, im_gt, im_other = data['lq'], data['gt'], data['other']
                else:
                    im_lq = data['lq']
                    im_other = data['other']
                num_iters = 0
                model_kwargs = {'lq': im_lq, 'other': im_other}
                model_kwargs['lq'] = self.encode_first_stage(model_kwargs['lq'], up_sample=False)
                model_kwargs['other'] = self.encode_first_stage(model_kwargs['other'], up_sample=False)
                tt = torch.tensor(
                ...
                if (ii + 1) % self.configs.train.log_freq[2] == 0:
                    current_datetime = datetime.datetime.now().strftime("%Y-%m-%d-%H-%M")
                    self.logger.info(f'{current_datetime}: Validation: {ii+1:02d}/{num_iters_epoch:02d}...')
            ...
            if 'gt' in data:
                ...
                current_datetime = datetime.datetime.now().strftime("%Y-%m-%d-%H-%M")
                self.logger.info(f'{current_datetime}: Validation Metric: PSNR={mean_psnr:5.2f}, LPIPS={mean_lpips:6.4f}...')
...
def replace_nan(im_lq, im_gt, im_other):
    '''
    Input:
        im_lq, im_gt, im: b x c x h x w
    '''
    if torch.isnan(im_lq).sum() > 0:
        valid_index = []
        im_lq = im_lq.contiguous()
        for ii in range(im_lq.shape[0]):
            if torch.isnan(im_lq[ii,]).sum() == 0:
                valid_index.append(ii)
        assert len(valid_index) > 0
        im_lq, im_gt, im = im_lq[valid_index,], im_gt[valid_index,], im_other[valid_index,]
        flag = True
    else:
        flag = False
    return im_lq, im_gt, im_other, flag
