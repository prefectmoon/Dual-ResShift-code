...
class SuperResModel(UNetModel):
    ...
    def __init__(self, image_size, in_channels, *args, **kwargs):
        ...
        print("SuperResModel image_size：", image_size)
...
if __name__ == "__main__":
    ...
    # print(model)
    x_t = torch.rand(16,3,64,64)
    t = torch.rand(16)
    low = torch.rand(16,3,64,64)
    other = torch.rand(16,3,64,64)
    model_kwargs = {'low_res': low, 'other': other}
    print(x_t.shape,t.shape,model_kwargs['low_res'].shape,model_kwargs['other'].shape)
    com_h1, com_h2, com_h3, dist_h1, dist_h2, dist_h3, model_output = model(x_t, t, **model_kwargs)
    print(com_h1.shape, com_h2.shape, com_h3.shape, dist_h1.shape, model_output.shape)
    ...
    # import cv2
    #
    # img = cv2.imread(r'E:\Aneurysm\resshift\train\S10110_origin_0.jpg')
    # print(img.shape)
    # #S10110_origin_0.jpg (256, 256, 3)
    #
    # # 提取第一个通道
    # channel1 = img[:, :, 0]
    #
    ...
    #
    # print(channel1.shape)  # 输出 (256, 256, 1)
    # cv2.imwrite(r'E:\code\ResShift-unet\temp_img\t.jpg', channel1)
    '''
    x_t = torch.rand(16, 1, 224, 224)
    t = torch.rand(16)
    low = torch.rand(16, 1, 224, 224)
    other = torch.rand(16, 1, 224, 224)
    model_kwargs = {'low_res': low, 'other': other}

    torch.Size([16, 1, 64, 64]) torch.Size([16]) torch.Size([16, 1, 64, 64]) torch.Size([16, 1, 64, 64])
    torch.Size([16, 144, 2, 2]) torch.Size([16, 144, 2, 2]) torch.Size([16, 144, 2, 2]) torch.Size([16, 144, 2, 2]) torch.Size([16, 2, 64, 64])
    '''