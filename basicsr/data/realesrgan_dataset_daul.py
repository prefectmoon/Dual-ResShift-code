...
def sort_key(filepath):
    match = re.match(r'.*\\S(\d+)_slice_(\d+)\.jpg', filepath)
    if match:
...
class RealESRGANDataset(data.Dataset):
    ...
    def __init__(self, opt, imgs,  mode='training'):
        ...
        self.paths = []
        # self.paths1.extend(sorted(os.listdir(Path(current_dir)), key=sort_key))
        # if 'dir_paths' in opt:
        #     # self.paths.extend(sorted(os.listdir(opt['dir_paths']), key=sort_key))
        ...
        # self.paths = sorted(self.paths, key=sort_key)
    ...
    def degrade_fun(self, conf_degradation, im_gt, kernel1, kernel2, sinc_kernel):
        ...
        return {'lq':out.contiguous(), 'gt':im_gt}
